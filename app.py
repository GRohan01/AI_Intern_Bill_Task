...
os.environ["TESSDATA_PREFIX"] = r"D:\All_programs\AI_INTERN_BILL_TASK\tessdata"
...
consumer_keywords = [
    "Consumer No", "Consumer Number", "Account No",
...
mobile_keywords = [
    "Mobile No", "Mobile Number", "Mob No",
    "मोबाईल/इंमेल", "मोबाइल नंबर"
]

units_keywords = [
    "Units", "Units Consumed", "Consumption",
    "युनिट", "एकूण युनिट", " B ) "
]

amount_keywords = [
    "Amount", "Bill Amount", "या तारखे पर्यंत रू. ",
    "पूर्णांक देयक (स)", "देयक रक्कम रु : "
]
...
# Extract name using Mobile Number position
        ...
        for keyword in mobile_keywords:
            if keyword.lower() in line.lower():
                ...
                if i + 1 < len(lines):
                    ...
                    next_line = re.sub(r'[^a-zA-Z\u0900-\u097F\s]', '', next_line)

                    if next_line and not re.search(r'\d', next_line):
                        if not any(word in next_line.lower() for word in ["bill", "date", "amount"]):
                            return next_line
...
uploaded_file = st.file_uploader("Upload Light Bill PDF", type=["pdf"])
